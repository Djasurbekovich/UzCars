...
function render(arr) {
  arr.forEach((el) => {
    let html = `
        <div class="card" style="width: 18rem; margin: 10px 0; box-shadow: 1px 1px 10px rgb(50, 50, 50)">
        <img src=${
          el.image.img1
        } style="width: 17.9rem;height: 14rem; object-fit: cover"class="card-img-top" alt="...">
        <div class="card-body">
          ...
          <button id=${el.id} class="btn corzinka mt-1 ${
      corzinkaArr.includes(el) ? "btn-success" : "btn-danger"
    } w-100">
          ${corzinkaArr.includes(el) ? "Saved" : "Save basket"}
          </button>
        </div>
      </div>
        `;
...
function renderCorzinka(arr) {
  arr.forEach((el) => {
    let html = `
        <div class="card col-4" style="width: 18rem; box-shadow: 1px 1px 10px white">
        <img src=${el.image.img1} style="width: 17.9rem;height: 14rem; object-fit: cover"class="card-img-top" alt="...">
        <div class="card-body">
          <h5 class="card-title">${el.name}</h5>
          <p class="card-text">Some quick example text to build on the card title and make up the bulk of the card's content.</p>
          <div class="d-flex align-items-center justify-content-between">
          ...
          <button id=${el.id} class="btn corzinka mt-1 btn-danger w-100">Delete basket</button>
        </div>
      </div>
        `;
